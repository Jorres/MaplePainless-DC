...
bool GlucoseSyrup::loadFormula(const char *filename) {
...
int GlucoseSyrup::getVariablesCount() { return solver->nVars(); }

// Get a variable suitable for search splitting
vector<int> GlucoseSyrup::getDivisionVariables(
    int k) // NOTE: ignoring values of k > 1 for now
{
  Lit res;
...
void GlucoseSyrup::setPhase(const int var, const bool phase) {
  solver->setPolarity(var - 1, phase);
}

// Bump activity for a given variable
void GlucoseSyrup::bumpVariableActivity(const int var, const int times) {
...
void GlucoseSyrup::setSolverInterrupt() { solver->interrupt(); }

// Diversify the solver
void GlucoseSyrup::diversify(int id) {
...
void GlucoseSyrup::unsetSolverInterrupt() { solver->clearInterrupt(); }

// Solve the formula with a given set of assumptions
// return 10 for SAT, 20 for UNSAT, 0 for UNKNOWN
SatResult GlucoseSyrup::solve(const vector<int> &cube, int nConflicts) {
...
void GlucoseSyrup::addClause(ClauseExchange *clause) {
  clausesToAdd.addClause(clause);

  setSolverInterrupt();
}

void GlucoseSyrup::addLearnedClause(ClauseExchange *clause) {
  if (clause->size == 1) {
...
void GlucoseSyrup::addClauses(const vector<ClauseExchange *> &clauses) {
  clausesToAdd.addClauses(clauses);

  setSolverInterrupt();
}

void GlucoseSyrup::addInitialClauses(const vector<ClauseExchange *> &clauses) {
...
void GlucoseSyrup::addLearnedClauses(const vector<ClauseExchange *> &clauses) {
...
void GlucoseSyrup::getLearnedClauses(vector<ClauseExchange *> &clauses) {
  clausesToExport.getClauses(clauses);
}

void GlucoseSyrup::increaseClauseProduction() { glueLimit++; }

void GlucoseSyrup::decreaseClauseProduction() { glueLimit--; }

SolvingStatistics GlucoseSyrup::getStatistics() {
...
std::vector<int> GlucoseSyrup::getModel() {
...
ClauseExchange *GlucoseSyrup::getFinalAnalysis() {
  ClauseExchange *out_cls = ClauseManager::allocClause(solver->conflict.size());

...
template <typename T> static bool getHeadFromVector(vector<T *> &vec, T **ws) {
  if (vec.size() == 0)
    return false;
  *ws = vec[0];
  vec.erase(vec.begin());
...
template <typename T> static bool removeFromVector(vector<T *> &vec, T *ws) {
...
template <typename T> static bool addOnceToVectorTail(vector<T *> &vec, T *ws) {
  for (int i = 0; i < vec.size(); i++) {
    if (vec[i] == ws)
      return false;
  }
...
template <typename T> static bool addOnceToVectorHead(vector<T *> &vec, T *ws) {
  for (int i = 0; i < vec.size(); i++) {
    if (vec[i] == ws)
      return false;
  }
...
template <typename T> static bool doesVectorContain(vector<T *> &vec, T *ws) {
  for (int i = 0; i < vec.size(); i++) {
    if (vec[i] == ws)
      return true;
  }
...
void *mainMasterDivideAndConquer(void *arg) {
  ...
  while (globalEnding == false && dc->strategyEnding == false) {
    ...
    if (successGetJob) {
      ...
      double delta = dc->times[actualWorker] + TIME_BEFORE_INTERRUPTING -
                     getAbsoluteTime();
      ...
      if (delta > 0) {
        log(2,
            "Master: workers are working for too short,I will sleep %f "
            " sec\n",
            delta);
...
DivideAndConquer::DivideAndConquer() {
  ...
  bool shouldUseInputVarsOnly =
      Parameters::getIntParam("n-input-vars", -1) != -1;

  if (shouldUseInputVarsOnly) {
    log(0, "Going to restrict splitting variable selection to input vars\n");
  } else {
    log(0,
        "Going to use variables for splitting freely, with no restrictions\n");
  }
...
void DivideAndConquer::solve(const vector<int> &cube) {
  actualCube = cube;
...
void DivideAndConquer::join(WorkingStrategy *current, SatResult res,
                            const vector<int> &model) {
  if (res == UNKNOWN_SELF_SPLIT) {
    log(0, "solver had solved something with a budget\n\n");
    ...
    log(1, "Slave %p is splitting by itself, (top) division variable is: %d\n",
        current, vars[0]);
    ...
    // TODO should I do that? seems like no
    // nWorkers++;
  ...
  //
    ...
    if (cloneStrategy == 2) {
      SolverInterface *cloneSolver = SolverFactory::cloneSolver(currentSolver);
  ...
  log(2,
      "DBG: The search is about to end. Post conditions: nWorkers: %d, "
      "workers-empty: %d, workQ-empty: %d\n",
...
void DivideAndConquer::setInterrupt() {
  for (size_t i = 0; i < slaves.size(); i++) {
    slaves[i]->setInterrupt();
  }
}

void DivideAndConquer::unsetInterrupt() {
  for (size_t i = 0; i < slaves.size(); i++) {
    slaves[i]->unsetInterrupt();
  }
}

void DivideAndConquer::waitInterrupt() {
  for (size_t i = 0; i < slaves.size(); i++) {
...
// Not used yet
vector<int> DivideAndConquer::getDivisionVariables(int k) {
  return vector<int>(1, 0);
}

// Not used yet
void DivideAndConquer::setPhase(int var, bool value) {}

// Not used yet
void DivideAndConquer::bumpVariableActivity(int var, int times) {}
